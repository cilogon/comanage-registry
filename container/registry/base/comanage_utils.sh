...
function comanage_utils::consume_injected_environment() {
    ...
    injectable_config_vars=( 
        ...
        COMANAGE_REGISTRY_OIDC_ALLOWLISTED_CLAIMS
        COMANAGE_REGISTRY_OIDC_BLOCKLISTED_CLAIMS
        COMANAGE_REGISTRY_OIDC_CLIENT_ID
        COMANAGE_REGISTRY_OIDC_CLIENT_SECRET
        COMANAGE_REGISTRY_OIDC_CRYPTO_PASSPHRASE
        COMANAGE_REGISTRY_OIDC_FILTER_CLAIMS_EXPR
        ...
        COMANAGE_REGISTRY_OIDC_SESSION_TYPE
...
OIDCOAuthServerMetadataURL ${COMANAGE_REGISTRY_OAUTH_SERVER_METADATA_URL:-$COMANAGE_REGISTRY_OIDC_PROVIDER_METADATA_URL}
...
OIDCSessionType ${COMANAGE_REGISTRY_OIDC_SESSION_TYPE:-server-cache}
EOF
...
EOF
            ...
            if [[ -n "${COMANAGE_REGISTRY_OIDC_ALLOWLISTED_CLAIMS}" ]]; then
                cat >> $virtual_host_config <<EOF
OIDCWhiteListedClaims ${COMANAGE_REGISTRY_OIDC_ALLOWLISTED_CLAIMS}
EOF
            fi

            if [[ -n "${COMANAGE_REGISTRY_OIDC_BLOCKLISTED_CLAIMS}" ]]; then
                cat >> $virtual_host_config <<EOF
OIDCBlackListedClaims ${COMANAGE_REGISTRY_OIDC_BLOCKLISTED_CLAIMS}
EOF
            fi

            if [[ -n "${COMANAGE_REGISTRY_OIDC_FILTER_CLAIMS_EXPR}" ]]; then
                cat >> $virtual_host_config <<EOF
OIDCFilterClaimsExpr ${COMANAGE_REGISTRY_OIDC_FILTER_CLAIMS_EXPR}
EOF
            fi
