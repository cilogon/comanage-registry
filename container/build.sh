...
EXAMPLES
    ...
    $0 -s 1 --namespace=myorg registry mod_auth_openidc
        Build the Registry image with OIDC authentication, namespace myorg,
        and tag suffix 1. The full name of the image will have the format
        myorg/comanage-registry:LABEL-mod_auth_openidc-1

    $0 -s 1 --registry=server.my.org --namespace=myorg
            registry mod_auth_openidc
        Build the Registry image with OIDC authentication, namespace myorg,
        registry server.my.org, and tag suffix 1. The full name of
        the image will have the format
        server.my.org/myorg/comanage-registry:LABEL-mod_auth_openidc-1
    ...
    $0 -s 20220501 --label mylabel registry mod_auth_openidc
        Build the Registry image with OIDC authentication, tag suffix
